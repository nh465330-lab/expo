...
data class JsDebugSession(
  ...
  companion object {
    fun fromSessionWithChildren(value: SessionWithChildren): JsDebugSession =
      fromSessionWithMetrics(value.sessionWithMetrics, value.crashReportPayload)

    fun fromSessionWithMetrics(
      value: SessionWithMetrics,
      crashReportPayload: String? = null
    ): JsDebugSession =
      JsDebugSession(
        id = value.session.id,
        type = "main",
        ...
        crashReport = decodeJsonObject(crashReportPayload)
      )
  }
}
