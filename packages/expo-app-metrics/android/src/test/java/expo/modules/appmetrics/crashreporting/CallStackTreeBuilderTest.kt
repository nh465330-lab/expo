...
class CallStackTreeBuilderTest {
  ...
  @Test
  fun `leaves iOS-only frame fields null`() {
    val tree = CallStackTreeBuilder.fromStackTrace(arrayOf(element()))

    val frame = requireNotNull(tree.callStacks?.first()?.callStackRootFrames).first()
    assertEquals(null, frame.binaryName)
    assertEquals(null, frame.binaryUUID)
    assertEquals(null, frame.address)
    assertEquals(null, frame.offsetIntoBinaryTextSegment)
    assertEquals(null, frame.subFrames)
  }
